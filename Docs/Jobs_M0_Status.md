...
## Tests
- Header-only self-containment: `tests/SelfContain/Jobs_header_only.cpp` includes the contract alone and `static_assert`s POD and concept conformance.
- Smoke helper: `tests/Smoke/Subsystems/Jobs_smoke.cpp` initializes the JobsSystem with Null, submits individual and batched jobs that mutate counters, runs `ParallelFor`, verifies counters complete, and returns 0 on success (no `main`).
